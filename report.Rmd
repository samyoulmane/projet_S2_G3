...
# (rappel : wdbc2, c'est wdbc avec les diagnostics en numérique)
nbclusters <- NbClust(wdbc_scaled, method="kmeans")
wdbc2 <- wdbc2 %>% mutate(cluster_kmeans = nbclusters$Best.partition)
table(wdbc$cluster_kmeans, wdbc2$diagnosis)
fviz_cluster(nbclusters$Best.nc, data=wdbc2)
```

L'algorithme a reconnu deux clusters : le cluster 1 de 189 observations, et le cluster 2 qui en comprend 380.

## Hierarchical clustering

```{r h_clust}

wdbc_hclust <- dist(wdbc_scaled) %>% hclust("ward.D") %>% cutree(2)

table(wdbc_hclust, wdbc2$diagnosis)
...
wdbc2_test <- wdbc2_class[ntest,]
wdbc2_train <- wdbc2_class[-ntest,]
wdbc2_test$diagnosis <- wdbc2_test$diagnosis %>% as.numeric
wdbc2_train$diagnosis <- wdbc2_train$diagnosis %>% as.numeric
```

## Arbres de décisions
```{r}
wdbc_DT <- rpart(diagnosis ~ ., data=wdbc2_train)
rpart.plot(wdbc_DT)

dt.pred <- predict(wdbc_DT, newdata=wdbc2_test ,type="class")
table(dt.pred, wdbc2_test$diagnosis)

```

## k plus proches voisins
```{r}
wdbc_KNN <- knn(train = wdbc2_train, 
                test = wdbc2_test , 
                cl = wdbc2_train$diagnosis,
                k = 5, prob=TRUE)

dt.pred <- predict(wdbc_KNN, newdata = wdbc2_test , type = "class")
table(knn.pred, wdbc2_test$diagnosis)

```
## Random forest
```{r}

wdbc_RF <- randomForest(diagnosis ~ ., wdbc2_train, prox=TRUE)

dt.pred <- predict(wdbc_RF, newdata=wdbc2_test ,type="class")
table(dt.pred, wdbc2_test$diagnosis)
```
