...
wdbc2 <-wdbc[-1]
wdbc2$diagnosis <- revalue(wdbc$diagnosis, c("B"=0, "M"=1))
...
# On aura parfois besoin de réduire les données à cause du manque de puissance de calcul
wdbc_mini<- wdbc_scaled[, c(-1, -4, -11, -14, -21, -24)]
```
...
wdbc_km <- kmeans(wdbc_scaled, 2)

# Visualisation des clusters
fviz_cluster(data = wdbc_scaled, model_km)

# Table
table(wdbc_km$cluster)
table(wdbc_km$cluster, wdbc2$diagnosis)

# Taille des clusters
questionr::freq(model_km, valid = F)
...
questionr::freq(wdbc_hclust, val=FALSE)
...
wdbc_em <- Mclust(wdbc_scaled, G=2)


table(wdbc_em$classification, wdbc2$diagnosis)
...
table_DT <- predict(wdbc_DT, newdata=wdbc2_test ,type="class") %>% 
...
table_KNN <- table(wdbc_KNN, wdbc2_test$diagnosis)
...
wdbc_RF <- randomForest(diagnosis ~ ., wdbc2_train, prox=TRUE) %>% 
              predict(wdbc_RF, newdata=wdbc2_test ,type="class") %>%
              table(wdbc2_test$diagnosis)
```
